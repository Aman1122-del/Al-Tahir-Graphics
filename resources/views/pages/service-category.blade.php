    ...
    <nav class="mb-6 text-sm text-slate-600 p-4" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2">
            ...
            <li><a href="{{ route('service.show', $service->slug) }}"
                    class="hover:text-[--color-brand-blue] transition-colors">{{ $service->title }}</a></li>
            <li><span class="mx-2 text-slate-400">›</span></li>
    ...
    <div x-data="categorySamples()" x-init="initSamples({{ $samples->toJson() }})">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <input type="number" x-model.number="minPrice" @input="applyFilters()" min="0"
                            placeholder="Min"
                        ...
                        <input type="number" x-model.number="maxPrice" @input="applyFilters()" min="0"
                            placeholder="Max"
                            class="w-20 rounded border border-slate-300 px-2 py-1 focus:border-[--color-brand-blue] focus:outline-none focus:ring-1 focus:ring-[--color-brand-blue]/20">
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 text-sm">
                    <span class="text-slate-600"><span x-text="filtered.length"></span> of <span x-text="all.length"></span>
                        designs</span>
                    ...
                    <button class="text-[--color-brand-blue] hover:text-[--color-brand-orange] font-medium"
        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" data-aos="fade-up" data-aos-delay="100">
            <template x-for="(sample, idx) in paginated" :key="sample.id">
                <article
                    class="group relative overflow-hidden rounded-2xl bg-white shadow-md ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl focus-within:ring-2 focus-within:ring-[--color-brand-blue]">
                    <div class="relative aspect-square w-full overflow-hidden cursor-pointer" @click="openSample(sample)"
                        ...
                        <div class="hidden h-full w-full items-center justify-center bg-slate-100 text-xs text-slate-600">
                            Image missing</div>
                        <div
                            ...
                            <div class="absolute bottom-4 left-4 right-4">
                                <div
                                    class="flex items-center justify-center rounded-lg bg-white/90 py-2 text-sm font-medium text-[--color-brand-deepblue] backdrop-blur-sm">
                                    Click to Customize</div>
                            </div>
                        </div>
                        <div class="absolute top-3 left-3">
                            <template x-if="isNew(sample)"><span
                                    class="inline-flex rounded-full bg-green-500 px-2 py-1 text-xs font-medium text-white shadow-lg">New</span></template>
                        </div>
                        <div class="absolute bottom-3 right-3 rounded-lg bg-white/95 px-3 py-1.5 text-sm font-bold text-[--color-brand-deepblue] shadow-lg backdrop-blur-sm"
                            x-text="sample.formatted_price"></div>
                    </div>
                    <div class="p-4">
                        <h3 class="text-base font-bold text-[--color-brand-deepblue] group-hover:text-[--color-brand-blue] transition-colors"
                            x-text="sample.title"></h3>
                        <template x-if="sample.description">
                            <p class="mt-2 text-sm text-slate-600 line-clamp-2"
                                x-text="sample.description.length > 80 ? sample.description.substring(0,80)+'…' : sample.description">
                            </p>
                        </template>
                    </div>
                </article>
        ...
        function categorySamples() {
            return {
                ...
                initSamples(items) {
                    ...
                    const types = new Set();
                    this.all.forEach(s => {
                        if (s.sample_type) types.add(s.sample_type);
                    });
                ...
                applyFilters() {
                    ...
                    if (q) arr = arr.filter(s => (s.title || '').toLowerCase().includes(q) || (s.description || '')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('click', async function(e) {
                if (e.target.classList.contains('add-sample-to-cart')) {
                    ...
                    `.sample-qty[data-sample-id="${sampleId}"]`);
                    const quantity = parseInt(qtyInput?.value || 1);

                    // Local storage add
                    const cart = getLocalCart();
                    const existing = cart.find(i => i.service_id == serviceId && i.service_sample_id ==
                        sampleId && i.unit_price == unitPrice);
                    if (existing) {
                        existing.quantity += quantity;
                    } else {
                        cart.push({
                            service_id: parseInt(serviceId),
                            service_sample_id: parseInt(sampleId),
                            quantity,
