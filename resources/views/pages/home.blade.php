<style>
    ...
    /* mobile tap highlight ko remove karne ke liye */
    a.btn-secondary {
...
@section('content')
    <div class="min-h-screen">
                ...
                <div class="grid lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div class="text-center lg:text-left" x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 100)">
                        <h1 class="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 leading-tight mb-6 transition-all duration-700 ease-out"
                            ...
                            Professional
                            <span
                                class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-blue-800">Printing</span>
                            Services
                        </h1>
                        <p class="text-xl text-gray-600 mb-8 max-w-xl transition-all duration-700 delay-200 ease-out"
                            x-bind:class="loaded ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'">
                            High-quality wedding cards, visiting cards, flyers, banners, brochures, and posters with modern
                    ...
                    <div class="relative" x-data="{ imageLoaded: false }" x-init="setTimeout(() => imageLoaded = true, 300)">
                        <div class="relative rounded-2xl overflow-hidden shadow-2xl transform transition-all duration-1000 ease-out hover:scale-105"
                            ...
                            <img src="{{ asset('images/random.jpg') }}" alt="Professional printing services showcase"
                                class="w-full h-96 object-cover" />
                            <!-- Overlay with floating badge -->
                            <div
                                class="absolute bottom-4 left-4 bg-white/95 backdrop-blur-sm p-4 rounded-xl shadow-lg border border-white/20 transform transition-all duration-300 hover:scale-105">
        ...
        <section
            class="bg-white bg-[radial-gradient(60%_120%_at_50%_50%,hsla(0,0%,100%,0)_0,rgba(252,205,238,.5)_100%)] py-16  border-t  mt-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center mb-12">
                    ...
                    @foreach ($popularImages as $image => $title)
                        <div class="group relative overflow-hidden rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
                            ...
                            <div class="absolute bottom-2 left-2 right-2">
                                <h4
                                    class="text-white text-sm font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-2 group-hover:translate-y-0">
                                    {{ $title }}
                                </h4>
                            </div>
                        </div>
                ...
                <div class="grid md:grid-cols-3 gap-8">
                    @php
                        $steps = [
                            ...
                            [
                                'icon' =>
                                    '<svg class="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2h4a1 1 0 011 1v1a1 1 0 01-1 1h-1v13a2 2 0 01-2 2H6a2 2 0 01-2-2V7H3a1 1 0 01-1-1V5a1 1 0 011-1h4z"></path></svg>',
                                'title' => 'Upload & Customize',
                                'description' =>
                                    'Upload your design or work with our team to create something amazing.',
                            ],
                            [
                                'icon' =>
                    ...
                    @foreach ($steps as $index => $step)
                        <div class="text-center group" data-aos="fade-up" data-aos-delay="{{ $index * 150 }}">
                            <div class="relative mb-6">
                                <div
                                    class="w-16 h-16 mx-auto bg-blue-600 text-white rounded-full flex items-center justify-center group-hover:bg-blue-700 transition-colors duration-300 transform group-hover:scale-110">
                                    {!! $step['icon'] !!}
                                </div>
                                <div
                                    class="absolute -top-2 -right-2 w-6 h-6 bg-orange-500 text-white text-xs rounded-full flex items-center justify-center font-bold">
                                    {{ $index + 1 }}
                                </div>
                            </div>
                ...
                <div class="grid md:grid-cols-3 gap-8">
                    @php
                        $features = [
                            [
                                ...
                                'description' =>
                                    'True-to-life colors and crisp details using professional-grade printers and premium inks.',
                            ],
                            [
                                'icon' =>
                                ...
                                'description' =>
                                    'Select from matte, glossy, textured, and specialty premium papers for the perfect finish.',
                            ],
                            [
                                'icon' =>
                                ...
                                'description' =>
                                    'Fast turnaround with reliable delivery to your doorstep. We value your time as much as you do.',
                            ],
                        ];
                    @endphp
                    @foreach ($features as $index => $feature)
                        <div class="group bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
                            data-aos="fade-up" data-aos-delay="{{ $index * 100 }}">
                            <div
                                class="w-12 h-12 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center mb-6 group-hover:bg-blue-600 group-hover:text-white transition-colors duration-300">
                                {!! $feature['icon'] !!}
                            </div>
