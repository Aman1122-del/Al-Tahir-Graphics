    ...
    <!-- Samples Only -->
                    ...
                    @if($subCategories->count() > 0)
                        <div class="border-b border-slate-200">
                            <nav class="-mb-px flex flex-wrap gap-2" aria-label="Filter categories">
                                <button class="category-tab whitespace-nowrap border-b-2 border-[--color-brand-blue] px-4 py-2 text-sm font-medium text-[--color-brand-blue]"
                                        :class="activeCategory === 'all' ? 'border-[--color-brand-blue] text-[--color-brand-blue]' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'"
                                        @click="filterByCategory('all')"
                                        role="tab"
                                        aria-selected="true">
                                    Show All
                                    <span class="ml-2 rounded-full bg-slate-100 px-2 py-0.5 text-xs" :class="activeCategory === 'all' ? 'bg-[--color-brand-blue] text-white' : 'bg-slate-100 text-slate-600'" x-text="allSamples.length"></span>
                    </button>
                                @foreach($subCategories as $category)
                                    <button class="category-tab whitespace-nowrap border-b-2 border-transparent px-4 py-2 text-sm font-medium text-slate-500 hover:text-slate-700 hover:border-slate-300"
                                            :class="activeCategory === '{{ $category }}' ? 'border-[--color-brand-blue] text-[--color-brand-blue]' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'"
                                        ...
                                        @endphp
                                        <span class="ml-2 rounded-full bg-slate-100 px-2 py-0.5 text-xs" :class="activeCategory === '{{ $category }}' ? 'bg-[--color-brand-blue] text-white' : 'bg-slate-100 text-slate-600'">{{ $categoryCount }}</span>
                        </button>
                    @endforeach
                            </nav>
                </div>
            @endif
                    ...
                    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3" id="samplesGrid">
                        <template x-for="(sample, index) in paginatedSamples" :key="sample.id">
                            <article class="group relative overflow-hidden rounded-xl bg-white shadow-md ring-1 ring-black/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg cursor-pointer"
                                     @click="openSampleModal(sample)"
                                     :data-category="sample.sub_category || 'uncategorized'"
                                     role="button"
                                     tabindex="0"
                                ...
                                <div class="relative h-48 overflow-hidden">
                                    ...
                                    <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                                        <div class="absolute bottom-4 left-4 right-4">
                                            <div class="flex items-center justify-center rounded-lg bg-white/90 py-2 text-sm font-medium text-[--color-brand-deepblue] backdrop-blur-sm">
                                    ...
                                    <div class="absolute top-3 left-3 flex flex-col gap-1">
                                        <template x-if="isNewSample(sample)">
                                            <span class="inline-flex items-center rounded-full bg-green-500 px-2 py-1 text-xs font-medium text-white shadow-lg">
                                                New
                                            </span>
                                        </template>
                                        <template x-if="index < 3">
                                            <span class="inline-flex items-center rounded-full bg-[--color-brand-orange] px-2 py-1 text-xs font-medium text-white shadow-lg">
                                ...
                                <!-- Product Info -->
                                <div class="p-4">
                                    <h4 class="font-bold text-[--color-brand-deepblue] group-hover:text-[--color-brand-blue] transition-colors" x-text="sample.title"></h4>
                                    <template x-if="sample.description">
                                        <p class="mt-1 text-sm text-slate-600 line-clamp-2" x-text="sample.description.length > 60 ? sample.description.substring(0, 60) + '...' : sample.description"></p>
                                    </template>
                                    
                                    <!-- Category Badge -->
                                    <template x-if="sample.sub_category">
                                        <div class="mt-2">
                                            <span class="inline-flex items-center rounded-full bg-slate-100 px-2 py-1 text-xs font-medium text-slate-700" x-text="sample.sub_category"></span>
                                        </div>
                                    </template>
                                    <!-- Quick Add to Cart -->
                                    <div class="mt-3 flex items-center justify-between gap-2">
                                        ...
                                        <button class="btn-primary text-xs px-3 py-2 add-sample-to-cart"
                                                ...
                                                :data-unit-price="sample.unit_price"
                                                :disabled="isAddingToCart[sample.id]"
                                                @click.stop="addToCartDirectly(sample)"
                                                aria-label="Add to cart">
                                            <svg x-show="!isAddingToCart[sample.id]" class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l-1 12H6L5 9z"/>
                                            </svg>
                                            <svg x-show="isAddingToCart[sample.id]" class="w-3 h-3 mr-1 animate-spin" fill="none" viewBox="0 0 24 24">
            ...
            @else
                ...
                <div class="rounded-xl bg-gradient-to-br from-slate-50 to-white p-6 ring-1 ring-black/5">
                    ...
                    <p class="mb-4 text-sm text-slate-600">This service is ready for immediate ordering. Select your quantity and add to cart.</p>
